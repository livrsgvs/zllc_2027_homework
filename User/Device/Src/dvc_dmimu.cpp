...
void Class_DM_IMU::IMU_UpdateEuler(uint8_t* pData)
{
	...
	Data.yaw -= DM_IMU_YAW_OFFSET * testy;
...
/**
 * @brief TIM定时器中断定期检测IMU是否存活
 *
 */
void Class_DM_IMU::TIM1msMod50_Alive_PeriodElapsedCallback()
{
    //判断该时间段内是否接收过遥控器数据
    if (pretes == testy)
    {
        //遥控器断开连接
        DM_IMU_Status = DM_IMU_Status_DISABLE;
    }
    else
    {
        DM_IMU_Status = DM_IMU_Status_ENABLE;
    }
    pretes = testy;
}
